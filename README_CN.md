...
- 🤝 **多代理支持**：使用 --instances 同时管理多个 Claude 会话
...
# 启动 2 个 Claude 会话并开启监视模式（多代理）
ccmaster watch --instances 2

# 启动 3 个 Claude 会话，每个最多自动续写 50 次
...
使用 `--instances` 参数时，CCMaster 提供所有会话的统一视图：
...
输出示例：
```
🚀 正在 /Users/yourname/project 启动 3 个 Claude 会话
👁️  监视模式：开启 - 空闲后将自动续写

[1] 📍 会话 ID: 20240124_143022
[2] 📍 会话 ID: 20240124_143023
[3] 📍 会话 ID: 20240124_143024

[1][14:30:22] 🚀 在终端中启动 Claude (窗口: 4211, 标签页: 2)
[2][14:30:23] 🚀 在终端中启动 Claude (窗口: 4211, 标签页: 3)
[3][14:30:24] 🚀 在终端中启动 Claude (窗口: 4211, 标签页: 4)

🎯 所有会话已启动。正在监控...
按 [q] 退出，按 [w] 切换所有会话的监视模式

[1][14:30:25] ● 处理中
[2][14:30:25] ● 空闲
[3][14:30:25] ● 处理中
[1][14:30:26] ▶ 用户："创建 API 端点"
[3][14:30:26] ▶ 用户："编写测试"
[2][14:30:27] ▶ 自动续写 (1/∞)
[1][14:30:28] ● 工作中
[1][14:30:28] → 使用 Write
...
使用 `--instances` 参数时，CCMaster 可以管理多个 Claude 会话：

```
┌─────────────┐     ┌──────────────┐     ┌─────────────┐
│   CCMaster  │────▶│ Claude Code 1│────▶│   钩子 1    │
│    监控器   │     └──────────────┘     └─────────────┘
│             │     ┌──────────────┐     ┌─────────────┐
│             │────▶│ Claude Code 2│────▶│   钩子 2    │
│             │     └──────────────┘     └─────────────┘
